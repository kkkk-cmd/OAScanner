...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测致远OA config.jsp 敏感信息泄漏漏洞")
    try:
        ...
        if response.status_code == 200 and "IP" in response.text:
            print(now_time() + ' [SUCCESS]  存在致远OA config.jsp 敏感信息泄漏漏洞:{}'.format(exp_url))
        else:
            print(now_time() + " [WARNING]  可能不存在致远OA config.jsp 敏感信息泄漏漏洞")
    except:
