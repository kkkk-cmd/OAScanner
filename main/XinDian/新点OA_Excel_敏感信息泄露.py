...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测新点OA ExcelExport 敏感信息泄露漏洞")
    try:
        requests.packages.urllib3.disable_warnings()
        respones1 = requests.get(plani_url, headers=headers, verify=False)
        if respones1.status_code == 200:
            print(now_time() + ' [SUCCESS]  新点OA ExcelExport 敏感信息泄露漏洞存在{}'.format(plani_url))
        else:
            print(now_time() + ' [WARNING]  新点OA ExcelExport 敏感信息泄露漏洞不存在')
    except:
