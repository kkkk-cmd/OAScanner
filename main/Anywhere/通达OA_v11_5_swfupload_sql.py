...
'''
    ...
    print(now_time() + " [INFO]     正在检测通达OA v11.5 swfupload_new.php SQL注入漏洞")
    try:
        ...
        if upload.status_code == 200 and '不安全的SQL语句' in upload.text:
            print(now_time() + ' [SUCCESS]  可能存在POST_sql注入漏洞 请使用sqlmap尝试进一步利用')
            print(now_time() + ''' [SUCCESS]  请修改数据包为:
            ...
            print(now_time() + ' [WARNING]  不存在通达OA v11.5 swfupload_new.php SQL注入漏洞')
    except:
