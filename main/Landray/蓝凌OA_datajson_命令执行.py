...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测蓝凌OA datajson 命令执行漏洞")
    ...
    try:
        ...
        if "模拟通过" in respones.text:
            print(now_time() + ' [SUCCESS]  蓝凌OA OA datajson 命令执行漏洞存在')
            print(now_time() + ' [SUCCESS]  payload:{}'.format(exp_url))
        else:
            print(now_time() + ' [WARNING]  蓝凌OA datajson 命令执行漏洞可能不存在')
    except:
