...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测泛微OA E-Cology HrmCareerApplyPerView的sql注入漏洞")
    try:
        res = requests.get(url=target_url, headers=headers, verify=False, timeout=10)
        if res.status_code == 200:
            print(now_time() + " [SUCCESS]  存在HrmCareerApplyPerView的sql注入漏洞{}".format(target_url))
        else:
            print(now_time() + " [WARNING]  不存在HrmCareerApplyPerView的sql注入漏洞")
    except:
