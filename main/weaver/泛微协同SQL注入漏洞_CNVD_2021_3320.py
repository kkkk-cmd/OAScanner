...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测泛微 OA E-Cology_LoginSSO_SQL_CNVD-2021-33202")
    try:
        requests.packages.urllib3.disable_warnings()
        response = requests.get(url=exp_url, headers=headers, verify=False)
        if response.status_code == 200:
            print(now_time() + ' [SUCCESS]  存在泛微CNVD-2021-3320漏洞:{}'.format(exp_url))
        else:
            print(now_time() + " [WARNING]  不存在泛微CNVD-2021-3320漏洞")
    except:
