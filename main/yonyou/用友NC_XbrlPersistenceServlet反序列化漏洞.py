...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    print(now_time() + ' [INFO]     正在检测漏洞用友 NC XbrlPersistenceServlet反序列化')
    ...
    try:
        ...
        if response.status_code == 200:
            print(now_time() + ' [SUCCESS]  用友 NC XbrlPersistenceServlet反序列化, 可能存在漏洞'.format(url))
        else:
            print(now_time() + ' [WARNING]  用友 NC XbrlPersistenceServlet反序列化可能不存在')
    except:
